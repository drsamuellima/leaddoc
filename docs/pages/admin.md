...
That clinic’s chatbots. The list is the same lightweight chatbot query as the clinic workspace, so it appears immediately. **Set up with AI** creates a draft and impersonates into the setup wizard. **Add ready chatbot** creates a finished bot and opens the studio. **Edit** impersonates into studio or setup. **Delete** removes the bot (`adminDeleteChatbot`).
