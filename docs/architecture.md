...
2. The script injects a transparent iframe pointing at `/w/{key}` and pins it to the real viewport corner, even when the clinic site’s CSS would trap `position: fixed` or restyle every `iframe` (margin, width, height). The loader owns that box with `!important` pixel sizes so a WordPress theme cannot push the chat off-screen. It does not rewrite the iframe’s whole style on every scroll, which would cancel the chat page load. Closed, it is a small launcher (about 80×80) in the clinic-chosen corner (bottom right or bottom left). After a short beat the panel grows open from that launcher — a compact card on that same side, up to about 400×650, inset from the edges and capped to the viewport — not a full-height strip that covers the site logo. Closing it keeps the launcher only for the rest of that tab. The iframe tells the parent to resize (with a matching ease) and which side to use via `postMessage` (`source: "dentchat"`).
