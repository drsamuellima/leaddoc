...
import { applyPipelineToLead, demoPipelines, ensureOrgPipelines, matchPipeline, stageIdForStatus } from "./pipelines";
...
export async function readJsonStore(): Promise<StoreData> {
  return readRaw();
}

export async function mutateJsonStore<T>(fn: (data: StoreData) => T | Promise<T>): Promise<T> {
